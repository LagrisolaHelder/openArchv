...
import { Sun, Moon, Unlock, FolderArchive, CheckCircle2, AlertCircle, Info, ExternalLink, Layers, Heart, XCircle, ShieldCheck } from 'lucide-react';
...
function App() {
  ...
  // Listen for background worker updates
  useEffect(() => {
  ...
  // Saves choice to local disk state if checked
  ...
  const handleExtract = async () => {
    try {
      ...
      setStatus({ message: 'Initializing decompression layout matrix...', type: 'info' });

      // ✨ 2-Second Mandatory Loading Animation Lock before engaging the Rust backend file task
      setTimeout(async () => {
        try {
          ...
          setStatus({ message: `Extraction pipeline error: ${innerErr}`, type: 'error' });
          setIsExtracting(false);
        }
      }, 2000);
  ...
  const getStatusClasses = () => {
    ...
    return {
      box: "bg-zinc-100 border-zinc-200 text-zinc-800 dark:bg-zinc-900/40 dark:text-zinc-100 dark:border-zinc-800 rounded-xl",
      icon: isExtracting ? (
  ...
  return (
    <div className="absolute top-0 left-0 w-screen h-screen px-8 py-10 font-sans select-none transition-all duration-300 flex flex-col justify-between bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white">
      ...
      <div className="flex justify-center w-full my-auto py-4">
        <main className="w-full max-w-[380px] transition-all duration-300">
          ...
          {/* CONDITION 1: Render the Terms of Use Shield check framework first */}
          {!hasConfirmedTerms ? (
            <div className="p-6 border border-zinc-200 bg-white dark:border-zinc-900 dark:bg-zinc-950/60 rounded-xl shadow-sm text-center animate-fade-in">
              <div className="flex justify-center mb-3">
                <ShieldCheck size={28} className="text-zinc-900 dark:text-white" />
              </div>
              <h2 className="text-base font-bold tracking-tight mb-2 text-zinc-900 dark:text-white">Terms of Software Use</h2>
              <p className="text-[11px] leading-relaxed mb-4 text-zinc-600 dark:text-zinc-400 text-left">
                By operating openArchv, you acknowledge that decompressing large disk payloads performs local system file writing. You agree to use this binary utility at your own discretion.
              </p>
              ...
              <div className="flex items-center justify-start gap-2 mb-5 cursor-pointer text-left">
                <input 
                  ...
                  className="w-3.5 h-3.5 accent-zinc-900 dark:accent-yellow-400 cursor-pointer rounded border-zinc-300"
                />
                <label htmlFor="rememberChoice" className="text-[11px] font-semibold text-zinc-500 dark:text-zinc-400 cursor-pointer select-none">
                  Remember choice on this machine
                </label>
              </div>

              <button 
                onClick={handleAcceptTerms}
                className="w-full py-2.5 text-xs font-semibold rounded-xl transition-all duration-200 bg-zinc-900 text-white border border-zinc-900 hover:bg-zinc-800 dark:bg-white dark:text-black dark:border-white dark:hover:bg-yellow-400 dark:hover:border-yellow-400 shadow-sm"
              >
                Confirm & Access Software
              </button>
            </div>
          ) : (
            
            /* CONDITION 2: Render standard functional workspace once confirmed */
            <>
              {/* Status Alert Bar */}
              ...
              {isExtracting && (
                <div className="p-6 mb-6 border border-zinc-200 bg-white dark:border-zinc-900 dark:bg-zinc-950/60 flex flex-col items-center justify-center text-center shadow-sm rounded-xl">
